...
Outputs  : Two Verilog files - base_Wallace.v and tb_base_Wallace.v - and total number of stages
...
3. At every stage the resultant of CSA modules are kept such that the number in the ith row has its significant bits starting from i.
4. I used variable 'temp' to keep track of the length of these numbers and updated them after passing each level.
5. The time period of the clock is calculated taking the delay of a transistor to be 1 unit of time.
*************************************************************************************************************************************************/
...
void main()
{
    ...
    int a, b;        //Stores the sample values to be multiplied in the Test Bench
    ...
    v = fopen("base_Wallace.v", "w");
    tb = fopen("tb_base_Wallace.v", "w");
    ...
    if (N > 2) {
        ...
        fprintf(v, "\n\n\n\t//Stage 1:\n\treg res1;\n\twire [%d:0] w11,w12;\n\treg [%d:%d][%d:%d] t1 = 0;", N, N - 1, K, 2 * N - 1, K);
        ...
        for (i = 1; i < N - 2; i++)
        {
            ...
            //Stage K
            fprintf(v, "\n\n\t//Stage %d:\n\n\treg [%d:0] res%d;\n\treg [%d:%d][%d:%d] t%d = 0;", K, i, i + 1, N - 1, K, 2 * N - 1, K, i + 1);
    ...
    //First we shall identify the final two numbers to be added. We can assume that two numbers will definetely exist for any size N
    for (i = 0; i < N; i++)
    ...
    fprintf(v, "\n\n\t//Stage %d:\n\twire [%d:0] rest;//unnecessary wire\n\twire cp;\n\treg [%d:0] result; \n\n\tcla%d carry_adder(t%d[%d][%d:%d], t%d[%d][%d:%d], rest, cp);", K, d - 1, 2 * N- 1, d, K - 1, i, d + i, i + 1, K - 1, j, d + i, i + 1);

    if (j == i + 1)
        fprintf(v, "\n\n\talways@(posedge clk)\n\tresult = {rest[%d:0],t%d[%d][%d],res%d};", 2 * N - K - 1, K - 1, i, i, K - 1);
    else
        fprintf(v, "\n\n\talways@(posedge clk)\n\tresult = {rest[%d:0],t%d[%d][%d:%d],res%d};", 2 * N - K - 1, K - 1, i, j - 1, i, K - 1);
    ...
    printf("Enter two values, a and b, to be multiplied in the Test Bench: ");
    scanf("%d %d", &a, &b);

    //Finding the value of clk so that the stages are executed successfully
    clk = (20 > 6 * ceil(log(cla_t) / log(2))) ? 20 : 6 * ceil(log(cla_t) / log(2));


    //The verilog code for Test Bench is being stored in tb_base_Wallace.v
    ...
    //In base_Wallace.v, verilog code for Full Adder module is being dumped.
    fprintf(v, "\n\n\nmodule FullAdder(input a,b,c, output cout,s);\n\tassign cout = a&b|b&c|c&a;\n\tassign s = a^b^c;\nendmodule");

    //In base_Wallace.v, verilog code for Carry-Save Adder modules are being dumped. Different CSA modules are dumped based on the requirements
    for (i = 1; i < 2 * N; i++)
    {
        if (csa_t[i] == 0)
            continue;
    ...
    printf("Verilog file for %d-bit Wallace multiplier with %d stages of pipelining has been created and can be found as base_Wallace.v.\nTest Bench has been created and can be found as tb_base_Wallace.v\n", N, K);

    // In base_Wallace.v, verilog code for opr module is being dumped. Opr module is used to compute the resultant of two carry statuses for the CLA (like generate or kill)
    fprintf(v, "\n\n\nmodule opr(p,q,o);\n\tinput [1:0] p,q;\n\toutput [1:0] o;\n\n\tassign o[0] = p[1] | q[0]&p[0];\n\tassign o[1] = p[1] | q[1]&p[0];\nendmodule\n\n");

    // In base_Wallace.v, verilog code for Carry Look-ahead adder is being dumped. Here based on cla_t the size of the variables are given.
    N = cla_t;
    K = ceil(log(N) / log(2));
